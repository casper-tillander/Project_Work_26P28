...
def get_long_name(target_name):
    """Converts technical target names like G_DA_X_Target to descriptive labels."""
    name = target_name.replace("_Target", "")
    parts = name.split("_")
    
    mapping = {
        "DA": "Dominant Ankle",
        "DK": "Dominant Knee",
        "NDA": "Non-Dominant Ankle",
        "NDK": "Non-Dominant Knee",
        "X": "X-direction",
        "Y": "Y-direction"
    }
    
    readable_parts = [mapping.get(p, p) for p in parts if p != "G"]
...
def main():
    ...
    y_train_all = train_df[targets].to_numpy(dtype=np.float32)
    X_test = test_df[features].to_numpy(dtype=np.float32)
    y_test_all = test_df[targets].to_numpy(dtype=np.float32)

    # Determine primary target (Ankle)
    ...
    models = {}
    all_test_preds = {}
    print(f"\nTraining {len(targets)} models separately...")

    for i, t_name in enumerate(targets):
        long_name = get_long_name(t_name)
        print(f"\n" + "="*80)
        print(f"TARGET: {t_name} ({long_name})")
        print("="*80)
        ...
        y_train_curr = y_train_all[:, i]
        y_test_curr = y_test_all[:, i]
        
        m.fit(X_train, y_train_curr)
        models[t_name] = m
        
        # Predictions
        p_train = m.predict(X_train)
        p_test = m.predict(X_test)
        all_test_preds[t_name] = p_test
        
        # Train Metrics
        tr_r2 = r2_score(y_train_curr, p_train)
        tr_mse = mean_squared_error(y_train_curr, p_train)
        tr_rmse = np.sqrt(tr_mse)
        tr_mae = mean_absolute_error(y_train_curr, p_train)
        
        print(f"[OVERALL TRAIN] R2: {tr_r2:.4f} | RMSE: {tr_rmse:.4f} | MSE: {tr_mse:.4f} | MAE: {tr_mae:.4f}")
        
        # Test Metrics
        te_r2 = r2_score(y_test_curr, p_test)
        te_mse = mean_squared_error(y_test_curr, p_test)
        te_rmse = np.sqrt(te_mse)
        te_mae = mean_absolute_error(y_test_curr, p_test)
        
        print(f"[OVERALL TEST ] R2: {te_r2:.4f} | RMSE: {te_rmse:.4f} | MSE: {te_mse:.4f} | MAE: {te_mae:.4f}")
        ...
        for act in activities:
            ...
            y_test_act = y_test_curr[mask]
            pred_test_act = p_test[mask]
            ...
            print(f"{act.capitalize():<15} -> R2: {act_r2:>6.4f} | RMSE: {act_rmse:>6.4f} | MSE: {act_mse:>7.4f} | MAE: {act_mae:>6.4f}")
        
        print("="*80 + "\n")

    print(f"Selecting '{primary_target}' for final model export.")
    
    # Save primary target outputs
    model = models[primary_target]
    original_targets = targets
    targets_export = [primary_target]
    
    y_test_primary = y_test_all[:, original_targets.index(primary_target)].reshape(-1, 1)
    pred_test_primary = all_test_preds[primary_target].reshape(-1, 1)
    ...
    (OUTPUT_DIR / "regressor_target_names.json").write_text(json.dumps(targets_export, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "regressor_test_vectors.json").write_text(json.dumps(make_test_vectors(X_test, y_test_primary, pred_test_primary), indent=2), encoding="utf-8")

    print(f"Saved model outputs for {primary_target} to: {OUTPUT_DIR}")

    # Plotting
    print("Generating plots for all targets...")
    ...
    # Extract timestamps for predictions
    pred_times = []
    ...
    first_two_acts = test_df['Activity'].unique()[:2]

    for t_name in original_targets:
        long_name = get_long_name(t_name)
        raw_target_col = t_name.replace("_Target", "")
        print(f"Plotting {long_name}...")

        # Prepare predictions dataframe
        y_pred_t = all_test_preds[t_name]
        plot_df = pd.DataFrame({
            'Predicted Angle': y_pred_t,
        ...
        # Filter time
        plot_df = plot_df[~(plot_df['Activity'].isin([a.capitalize() for a in first_two_acts]) & (plot_df['Time (s)'] > 5.0))]
        raw_plot_df = raw_plot_df[~(raw_plot_df['Activity'].isin([a.capitalize() for a in first_two_acts]) & (raw_plot_df['Time (s)'] > 5.0))]
        ...
        for ax, act in zip(axes, plot_activities):
            ...
            ax.plot(act_raw['Time (s)'], act_raw[raw_target_col], 
                    color=palette["True Angle"], label="True Angle", linewidth=1.5, alpha=0.9)
        ...
        fig.supylabel(f"{long_name} (deg)", fontsize=12)
        fig.suptitle(f"True vs Predicted: {long_name}", fontsize=14, weight='bold')
        ...
        clean_name = t_name.lower().replace("_target", "")
        plot_path = OUTPUT_DIR / f"time_series_{clean_name}.png"
